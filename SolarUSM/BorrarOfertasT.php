...
function borrarTrabajo($conexion, $idTrabajo) {
    ...
    if (borrarSolicitudes($conexion, $idTrabajo)) {
        ...
        $sql = "DELETE FROM trabajos_transportes WHERE id_transportes = ?";
        if ($stmt = $conexion->prepare($sql)) {
            $stmt->bind_param("i", $idTrabajo);
            if ($stmt->execute()) {
                return true;
...
<body>
    ...
    <?php
        ...
        $sql = "SELECT id_transportes, nombre_trabajo, ubicacion, detalle_trabajo, fecha_inicio,  fecha_termino, valor_pagar, hora_inicio, hora_fin, capacidad_minima FROM trabajos_transportes WHERE rut_contratista = ?";
        if ($stmt = $conn->prepare($sql)) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<input type='hidden' name='id' value='" . $row['id_transportes'] . "'>";
